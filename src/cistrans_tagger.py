...
def get_unique_trans_snps(all_snps_df, params):
    ...
    unique_trans_snps = all_snps_df[all_snps_df["cis_trans"] == "trans"].drop_duplicates(
        subset=[params["snp_file_chrom_col"], params["snp_file_pos_col"]]
    )
...
def batch_annotate_trans_snps(trans_snps_df, genes_bed, params):
    ...
    trans_snps_bed = pybedtools.BedTool.from_dataframe(
        trans_snps_df[[params["snp_file_chrom_col"], params["snp_file_pos_col"], params["snp_file_pos_col"]]]
    )
...
def cistrans_gene_tagger(params, output_path):
    ...
    for i, seqid in enumerate(seqid_list, start=1):
        ...
        if not target_protein.empty:
            ...
            if len(snp_files) > 2:
                ...
                logging.warning(
                    f"Too many files found for SeqId {seqid}. Processing the first encountered file only. Files found: {', '.join(snp_files)}"
                )
            ...
            for file in snp_files:
                if ".log" not in file:
                    ...
                    proximal = (snp_df["dist_lower"] < params["proximality_threshold"]) | (
                        snp_df["dist_upper"] < params["proximality_threshold"]
                    )
                    snp_df["proximal"] = "N"
    ...
    gene_df = pd.read_csv(params["gene_pos_file_dir"], sep=r"\s+", header=0)

    # Ensuring chromosome is a string for sorting
    gene_df["CHR"] = gene_df["CHR"].astype(str)

    # Step 1: Filter Unique Trans SNPs
    unique_trans_snps = all_snps_combined_df[all_snps_combined_df["cis_trans"] == "trans"].drop_duplicates(
        subset=[params["snp_file_chrom_col"], params["snp_file_pos_col"]]
    )
    ...
    snps_bed = df_to_bed(
        unique_trans_snps, params["snp_file_chrom_col"], params["snp_file_pos_col"], params["snp_file_pos_col"]
    )
    ...
    closest_genes = snps_bed.closest(genes_bed, d=True, t="first").to_dataframe(
        names=["chr", "start", "end", "gene_chr", "gene_start", "gene_end", "gene_name", "score", "strand", "distance"]
    )

    # Creating a dictionary for mapping with keys as tuples (chromosome, position)
    snp_to_gene_map = dict(
        zip(zip(closest_genes["chr"].astype(str), closest_genes["start"]), closest_genes["gene_name"])
    )
    ...
    initial_cols = [
        params["prot_id_column_name"],
        params["snp_file_chrom_col"],
        params["snp_file_pos_col"],
        params["pval_column"],
        "cis_trans",
        "proximal",
        "dist_lower",
