...
@click.option('-f', '--input_file_format', required=True, type=click.Choice(['regenie', 'vcf'], case_sensitive=False), help='Input file format')
...
def cli_init(config_file, input_file, input_file_format, input_file_separator, quiet):
    ...
    report_file_path = Path(cm.save_path, cm.report_filename.replace('placeholder', input_file_name))
    with open(report_file_path, "w") as fp:
        fp.write(f"input file path: {input_file_path}")

    if input_file_path.exists():
        sm = SumstatsManager(input_file_path.as_posix(), input_file_format, input_file_separator)
    else:
        msg = f"{input_file_path} input file not found"
        exit(msg)

    run, params = cm.action('basic_check')
    if run:
        args = {k: v for k, v in params.items()}
        print(args)
        sm.mysumstats.basic_check(**args)

    run, params = cm.action('infer_build')
    if run:
    ...
    run, params = cm.action('fill_data')
    if run:
        args = {k: v for k, v in params.items()}
        sm.mysumstats.fill_data(**args)

    run, params = cm.action('harmonize')
    if run:
        args = {k: v for k, v in params.items()}
        sm.mysumstats.harmonize(**args)
        sm.mysumstats.flip_allele_status()

    run, params = cm.action('report_summary')
    if run:
        header = f"Summary:"
    ...
    run, params = cm.action('report_inflation_factors')
    if run:
        header = f"\nInflation factors:"
    ...
    run, params = cm.action('write_ldsc')
    if run:
        output_path = str(Path(cm.save_path, input_file_path.stem))
        args = {k: v for k, v in params.items()}
        sm.mysumstats.to_format(output_path, **args)

    run, params = cm.action('write_metal')
    if run:
        output_path = str(Path(cm.save_path, input_file_path.stem))
        args = {k: v for k, v in params.items()}
        sm.mysumstats.to_format(output_path, **args)

    run, params = cm.action('write_regenie')
    if run:
        output_path = str(Path(cm.save_path, input_file_path.stem))
        args = {k: v for k, v in params.items()}
        sm.mysumstats.to_format(output_path, **args)
